  ...
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="日期" prop="createdAt" width="180">
          ...
          <el-table-column align="left" label="sui objectID" prop="objectId" width="120" />
          <el-table-column align="left" label="名称" prop="name" width="120" />
          <el-table-column align="left" label="地址" prop="address" width="120" />
          <el-table-column align="left" label="图片" prop="imageUrl" width="120" />
    ...
    <el-drawer destroy-on-close :size="appStore.drawerSize" v-model="dialogFormVisible" :show-close="false" :before-close="closeDialog">
       <template #header>
          ...
          <el-form :model="formData" label-position="top" ref="elFormRef" :rules="rule" label-width="80px">
            <el-form-item label="sui objectID:"  prop="objectId" >
              <el-input v-model="formData.objectId" :clearable="true"  placeholder="请输入sui objectID" />
            </el-form-item>
            ...
            <el-form-item label="图片:"  prop="imageUrl" >
              <el-input v-model="formData.imageUrl" :clearable="true"  placeholder="请输入图片" />
            </el-form-item>
          </el-form>
    </el-drawer>
...
import {
  createCertificate,
  deleteCertificate,
  deleteCertificateByIds,
  updateCertificate,
  findCertificate,
  getCertificateList
} from '@/api/certificate/certificate.go'
...
const getTableData = async() => {
  const table = await getCertificateList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
  ElMessageBox.confirm('确定要删除吗?', '提示', {
    ...
    type: 'warning'
      ...
      const res = await deleteCertificateByIds({ IDs })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateCertificateFunc = async(row) => {
    const res = await findCertificate({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteCertificateFunc = async (row) => {
