  ...
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="操作" fixed="right" :min-width="appStore.operateMinWith">
            ...
            <el-button  type="primary" link icon="edit" class="table-button" @click="updateEmployeeFunc(scope.row)">编辑</el-button>
       ...
       <template #header>
          ...
          <el-form :model="formData" label-position="top" ref="elFormRef" :rules="rule" label-width="80px">
            ...
            <el-form-item label="CompanyId:"  prop="companyId" >
              <el-input v-model.number="formData.companyId" :clearable="true" placeholder="请输入CompanyId" />
            </el-form-item>
          </el-form>
    </el-drawer>
...
const getTableData = async() => {
  const table = await getEmployeeList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
// 更新行
const updateEmployeeFunc = async(row) => {
    const res = await findEmployee({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteEmployeeFunc = async (row) => {
